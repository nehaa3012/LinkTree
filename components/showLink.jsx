...
import { updateLink } from "@/actions/userAction";
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-2 w-full max-w-4xl mx-auto mt-8">
      {links.map((link) => (
        <Card
          key={link.id}
          className="group hover:shadow-lg transition-all duration-300 border-none bg-white/50 backdrop-blur-sm dark:bg-black/50 overflow-hidden relative"
        >
          ...
          <CardContent>
            ...
            <div className="pl-[44px] flex gap-2">
              <Button
                ...
                variant="outline"
                size="sm"
                className="flex-1 group-hover:bg-purple-50 dark:group-hover:bg-purple-900/20 group-hover:border-purple-200 transition-colors"
              >
                <Link
                  href={link.url}
                  target="_blank"
